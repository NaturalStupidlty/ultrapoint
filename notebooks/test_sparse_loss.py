...
uv_a_non_matches = correspondence_finder.create_non_correspondences(
    uv_a_matches, img_a_shape, num_non_matches_per_match=10, img_b_mask=None
)
uv_b_non_matches = correspondence_finder.create_non_correspondences(
    uv_b_matches, img_b_shape, num_non_matches_per_match=10, img_b_mask=None
)
...
match_loss, matches_a_descriptors, matches_b_descriptors = (
    PixelwiseContrastiveLoss.match_loss(
        image_a_pred, image_b_pred, matches_a.long(), matches_b.long()
    )
)
(
    non_match_loss,
...
homographies_H = np.stack(
    [scale_homography(homographies, image_shape, shift=(-1, -1)) for H in homographies]
)
homographies_H = np.stack(
    [
        scale_homography_torch(
            torch.tensor(homographies, dtype=torch.float32), image_shape, shift=(-1, -1)
        )
...
def get_coor_cells(Hc, Wc, cell_size, device="cpu", uv=False):
    ...
    if uv:
        coor_cells = torch.stack(
            (coor_cells[:, 1], coor_cells[:, 0]), dim=1
        )  # (y, x) to (x, y)
...
filtered_points, mask = filter_points(
    coor_cells, torch.tensor([Wc, Hc]), return_mask=True
)
...
def descriptor_loss_sparse(
    ...
    **config
    ...
    def get_non_matches_corr(
        img_b_shape, uv_a, uv_b_matches, num_masked_non_matches_per_match=10
    ):
        ## sample non matches
    ...
    uv_b_matches, mask = filter_points(
        uv_b_matches, torch.tensor([Wc, Hc]), return_mask=True
    )
    uv_a = uv_a[mask]

    # crop to the same length
    choice = crop_or_pad_choice(
        uv_b_matches.shape[0], num_matching_attempts, shuffle=True
    )
    ...
    non_match_loss = get_non_match_loss(
        image_a_pred, image_b_pred, non_matches_a, non_matches_a
    )
...
loss = batch_descriptor_loss_sparse(
    descriptors, descriptors_warped, torch.tensor(homographies, dtype=torch.float32)
)
