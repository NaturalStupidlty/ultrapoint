...
import cv2
from src.ultrapoint.utils.utils import warp_points_np
...
def sample_homography(inv_scale=3):
    ...
    img_offset = np.array(
        [(-1, -1), (-1, offset_r), (offset_r, -1), (offset_r, offset_r)]
    )
...
def test_sample_homography():
    batch_size = 30
    filename = "../assets/configs/superpoint_coco_train.yaml"
    import yaml
    ...
    if test_corner_def:
        ...
        from src.ultrapoint.utils.utils import sample_homography
...
def test_valid_mask():
    ...
    for i in range(1):
        ...
        cell_size = 8
        mask_valid = labels2Dto3D(
            mask_valid.view(batch_size, 1, mask_valid.shape[1], mask_valid.shape[2]),
            cell_size=cell_size,
        )
