...
from src.ultrapoint.models.homographies import sample_homography
...
class PatchesDataset(data.Dataset):
    ...
    def __init__(self, transform=None, **config):
        ...
        sequence_set = []
        for img, img_warped, mat_hom in zip(
    ...
    def __getitem__(self, index):
        ...
        def _preprocess(image):
            ...
            image = cv2.resize(
                image, (self.sizer[1], self.sizer[0]), interpolation=cv2.INTER_AREA
            )
        ...
        homography = _adapt_homography_to_preprocessing(
            image_original, sample["homography"]
        )
        sample = {
            "image": image,
    ...
    def _init_dataset(self, **config):
        ...
        base_path = Path(os.getenv("DATA_PATH"), dataset_folder)
