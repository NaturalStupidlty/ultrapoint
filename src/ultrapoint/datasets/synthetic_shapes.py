...
class SyntheticShapes(BaseDataset):
    ...
    def dump_primitive_data(self, primitive, tar_path, config):
        ...
        tf.logging.info(f"Generating .tar file for primitive {primitive}.")
        ...
        tf.logging.info(f".tar file dumped to {tar_path}.")
    ...
    def _init_dataset(self, **config):
        ...
        basepath = Path(
            DATA_PATH,
            f"synthetic_shapes{'_' + config['suffix'] if config['suffix'] is not None else ''}",
        )
        basepath.mkdir(parents=True, exist_ok=True)

        splits = {
            s: {"images": [], "points": []} for s in ["training", "validation", "test"]
        }
        for primitive in primitives:
            ...
            # Untar locally
            tf.logging.info(f"Extracting archive for primitive {primitive}.")
    ...
    def _get_data(self, filenames, split_name, **config):
        ...
        def _gen_shape():
            ...
            while True:
                ...
                yield (
                    np.expand_dims(image, axis=-1).astype(np.float32),
        ...
        if config["on-the-fly"]:
            ...
            data = data.map(
                lambda i, c: pipeline.downsample(i, c, **config["preprocessing"])
            )
        else:
            ...
            data = data.map(
                lambda image, points: (
                    _read_image(image),
                    tf.py_func(_read_points, [points], tf.float32),
                )
            )
