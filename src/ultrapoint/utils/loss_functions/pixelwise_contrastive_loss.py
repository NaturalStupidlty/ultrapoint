...
class PixelwiseContrastiveLoss(object):
    ...
    def match_loss(
        ...
        if method == "2d":
            ...
            def sampleDescriptors(image_a_pred, matches_a, mode, norm=False):
                ...
                matches_a_descriptors = F.grid_sample(
                ...
                if norm:
                    dn = torch.norm(
                        matches_a_descriptors, p=2, dim=1
                    )  # Compute the norm of b_descriptors
            ...
            # matches_a_descriptors = matches_a_descriptors.squeeze().transpose(0,1)
            norm = False
            matches_a_descriptors = sampleDescriptors(
                image_a_pred, matches_a, mode, norm=norm
            )
            matches_b_descriptors = sampleDescriptors(
                image_b_pred, matches_b, mode, norm=norm
            )
        else:
            num_matches = matches_a.size()[0]
        ...
        if dist == "cos":
            ...
            match_loss = torch.clamp(
                M - (matches_a_descriptors * matches_b_descriptors).sum(dim=-1), min=0
            )
    ...
    def non_match_descriptor_loss(
        ...
        if dist == "cos":
            non_match_loss = (
                non_matches_a_descriptors * non_matches_b_descriptors
            ).sum(dim=-1)
        else:
            non_match_loss = (
                non_matches_a_descriptors - non_matches_b_descriptors
    ...
    def non_match_loss_with_l2_pixel_norm(
        ...
        non_match_descriptor_loss, num_hard_negatives, _, _ = (
