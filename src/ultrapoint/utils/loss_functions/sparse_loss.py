...
def descriptor_loss_sparse(
    ...
    **config
    ...
    def get_match_loss(
        image_a_pred, image_b_pred, matches_a, matches_b, dist="cos", method="1d"
    ):
    ...
    def get_non_match_loss(
        image_a_pred, image_b_pred, non_matches_a, non_matches_b, dist="cos"
    ):
        ## non matches loss
        (
            non_match_loss,
    ...
    from src.ultrapoint.utils.utils import crop_or_pad_choice
    from src.ultrapoint.utils.utils import normPts
    ...
    uv_b_matches = warp_coor_cells_with_homographies(
        uv_a, homographies_H.to("cpu"), uv=True, device="cpu"
    )
    #
    ...
    uv_b_matches, mask = filter_points(
        uv_b_matches, torch.tensor([Wc, Hc]).to(device="cpu"), return_mask=True
    )
    ...
    choice = crop_or_pad_choice(
        uv_b_matches.shape[0], num_matching_attempts, shuffle=shuffle
    )
...
def batch_descriptor_loss_sparse(
    descriptors, descriptors_warped, homographies, **options
):
    loss = []
    pos_loss = []
    neg_loss = []
